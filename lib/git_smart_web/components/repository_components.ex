defmodule GitSmartWeb.RepositoryComponents do
  ...
  def repository_item(assigns) do
    ~H"""
    <%= if @link == "#" do %>
      <div class="flex space-x-3 p-2 bg-sky-50 hover:brightness-95 transition-all rounded-md -z-10">
    ...
    <% else %>
      <.link
        navigate={@link}
        class="flex space-x-3 p-2 bg-sky-50 hover:brightness-95 transition-all rounded-md -z-10"
      >
        <img src={@repository.avatar_url} class="h-6 w-6 rounded-full" />
        <div class="flex-1 space-y-4">
          <div class="flex items-center justify-between">
